...
/**
 * Smoke test : vérifie que le lanceur du TP3 s'affiche correctement et que les --add-opens
...
class AppTest {
  ...
  void le_lanceur_affiche_le_titre_du_tp(FxRobot robot) {
    Label titre = robot.lookup("#titre-tp3").queryAs(Label.class);
    assertThat(titre).as("le label titre du TP3 doit être présent").isNotNull();
    assertThat(titre.getText()).as("le titre doit mentionner TP3").contains("TP3");
  }
